...
public interface YdrometraDatesRepository extends JpaRepository<Dual, Long> {
    public static final String qry =
            ...
            "    select    ydr_fconsumers.id                                     as id,\n" +
                ...
                "\n" +
                "                       LEFT OUTER JOIN YDR_FSECTORS ON ( ydr_fconsumers.RMA_TOMEAS = YDR_FSECTORS.SECT_CODE)       \n" +
    "               where YDR_FCONSUMERS.id in ( select maa                                                                     \n" +
                "                                from YDR_STATUS_CHANGE                                                         \n" +
                "                                 where to_char(YDR_STATUS_CHANGE.CHANGE_DATE,'yyyymmdd') between :fromdate and :todate  \n" +
                "                               )                                                                                       \n" +
                "       and not exists (select 1                                                                                  \n" +
                "                       from ydr_fydromet                                                                       \n" +
                "                        where ydr_fydromet.RYDR_MAA = YDR_FCONSUMERS.ID                                         \n" +
                "                          and (to_char(ydr_fydromet.rydr_date,'yyyymmdd') between :fromdate and :todate)           \n" +
                "                        )"                        ;
